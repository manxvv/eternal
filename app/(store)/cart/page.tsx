...
//         body: JSON.stringify({
//           orderItems: items,
//           totalPrice: totalPrice + (totalPrice >= 999 ? 0 : 99),
//         }),
//       });
...
//         setOrdered(true);
//       } else {
//         alert("Atelier Error: We could not process this order. Please check your connection.");
//       }
//     } catch (error) {
...
//             Your curated collection awaits. Please sign in to manage your ritual items and proceed to checkout.
//           </p>
...
//                           <button onClick={() => updateQty(item.id, item.quantity - 1)} className="w-10 h-10 flex items-center justify-center text-brand-blue hover:bg-brand-blue/5">-</button>
//                           <span className="w-10 text-center text-xs font-bold text-brand-blue">{item.quantity}</span>
//                           <button onClick={() => updateQty(item.id, item.quantity + 1)} className="w-10 h-10 flex items-center justify-center text-brand-blue hover:bg-brand-blue/5">+</button>
//                         </div>
//                         <button onClick={() => removeItem(item.id)} className="text-[10px] uppercase tracking-widest text-red-800/40 hover:text-red-800 font-bold underline underline-offset-4">Remove</button>
//                       </div>
//                     </div>

//                     <p className="font-cormorant text-2xl text-brand-blue">₹{(item.price * item.quantity).toLocaleString()}</p>
//                   </div>
//                 ))}
//               </div>
//             </div>
...
//                 <h4 className="font-cormorant text-2xl text-brand-blue mb-8 border-b border-brand-blue/5 pb-4">Summary</h4>
...
//                   className={`w-full py-5 uppercase tracking-[0.2em] text-[11px] font-bold shadow-xl transition-all mb-4
//                     ${isProcessing ? 'bg-gray-200 text-gray-500' : 'bg-brand-blue text-brand-gold hover:bg-brand-blue/90 shadow-brand-blue/10'}
//                   `}
//                 >
